                                ...
                                <div class="container-fluid px-3">
                                    <div class="row mb-3">
                                        <div class="col d-flex justify-content-end pb-3">
                                            <a href="{{ route('users.create') }}" class="btn btn-success ml-3 text-white" wire:navigate><i class="fa fa-plus" aria-hidden="true"></i>{{ __(' Create New User') }}</a>
                                        </div>
                                    </div>
                                            ...
                                            <tbody>
                                                @foreach ($users as $user)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{ route('users.edit', $user->userId) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                                                            <button class="btn btn-warning" wire:click.prevent="resetPassword('{{ $user->userId }}')"><i class="fas fa-rotate"></i></button>
                                                            <button class="btn btn-danger" wire:click.prevent="deleteConfirm('{{ $user->userId }}')"><i class="fas fa-trash"></i></button>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
